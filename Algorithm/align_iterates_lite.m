function [aligned_its] = align_iterates_lite( iterates,ind,val,sub_pix)
%Jclark  
...
switch ind
    ...
    case -1
        ...
        for qq = 1:n_its
            ...
            beta=check_conj_ref(abs(alpha),abs(beta));
            ...
            if isempty(val) == 1
                ...
                if dims == 3, aligned_its(:,:,qq)=zero_phase_cc(beta_s,val);end
                if dims == 4, aligned_its(:,:,:,qq)=zero_phase_cc(beta_s,val);end
            end
...
function [array1 phi] = zero_phase_cc(array1,array2)
%jclark
%will set array1 avg phsae to array2

cc=conj(array1).*array2;

ccval=sum(cc(:));

phi=angle(ccval);

array1=array1.*exp(i*phi);
